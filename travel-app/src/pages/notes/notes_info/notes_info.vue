<template>
  <view>
    <uni-card style="border-radius: 20rpx; " :is-shadow="false" :border="false" margin="10rpx" spacing="0rpx"
      ...
      <view>
        <view
          style="margin: 30rpx; display: flex; flex-direction: row; align-items: center; justify-content: space-between;">
          <uni-title type="h5" :title="notes.username+' '+notes.createdTime" />
          <view>
            <button v-show="showForm===0" style="border-radius: 50rpx;" type="primary" size="mini" @click="add()">
              编写评论
            </button>
            <button v-show="showForm===1" style="border-radius: 50rpx;" type="warn" size="mini" @click="del()">
              取消评论
            </button>
          </view>
        </view>
